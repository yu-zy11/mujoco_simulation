...
class MujocoCommand(object):
    __slots__ = ["q_des_abad", "q_des_hip", "q_des_knee", "qd_des_abad", "qd_des_hip", "qd_des_knee", "kp_abad", "kp_hip", "kp_knee", "kd_abad", "kd_hip", "kd_knee", "tau_abad_ff", "tau_hip_ff", "tau_knee_ff"]

    __typenames__ = ["float", "float", "float", "float", "float", "float", "float", "float", "float", "float", "float", "float", "float", "float", "float"]
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(MujocoCommand._get_packed_fingerprint())
    ...
    def decode(data):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != MujocoCommand._get_packed_fingerprint():
            raise ValueError("Decode error")
        return MujocoCommand._decode_one(buf)
    ...
    def _get_hash_recursive(parents):
        if MujocoCommand in parents: return 0
        tmphash = (0x80d6a48a21130a32) & 0xffffffffffffffff
    ...
    def _get_packed_fingerprint():
        if MujocoCommand._packed_fingerprint is None:
            MujocoCommand._packed_fingerprint = struct.pack(">Q", MujocoCommand._get_hash_recursive([]))
        return MujocoCommand._packed_fingerprint
